---
name: write-module-transition
description: Create module transitions for complex workflow orchestration
---
...
- **Decompose, then compose in WSL.** Never write a large method that does
  several things. Split fetch / validate / transform / respond into separate
  single-purpose transitions and chain them as sequential steps in a `.wsl` /
  `.swsl` workflow. Orchestration belongs in the declarative WSL layer, not
  inside one Go function. With `engine@v1.0.0` the WSL layer cannot evaluate
  `when`/`if` at runtime or loop over revisited states, so keep only genuine
  **branching/looping** inside a single focused Go transition — decompose
  everything around it. Full rationale: <https://docs.kuetix.com/docs/wsl/composition>.
