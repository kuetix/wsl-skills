---
name: write-service-transition
description: Create Go service transitions (action handlers)
---
...
## Most important rule: decompose, then compose in WSL

**Do not write large, monolithic transition methods.** If a method does several
things (fetch *and* validate *and* transform *and* respond), split it into
several small, single-purpose transitions and compose them as sequential steps
in a `.wsl` / `.swsl` workflow — do not bury the flow inside one Go function.
Orchestration belongs in the declarative WSL layer; the Go transitions are the
small, reusable, independently-testable leaves.

Before writing a method, ask: *can I describe it in one sentence without "and"?*
If not, split it.

**Where to stop (engine@v1.0.0):** decompose along **sequential** boundaries
only. The current engine cannot evaluate `when`/`if` at runtime, and revisited
states do not re-evaluate accumulating data — so genuine **branching** (multi-way
routing) and **looping** must stay inside a single focused Go transition for now.
Decompose everything *around* that control flow. (See `std-ai`: `ai/prompt.Prompt`
routes providers and `ai/agent.Run` holds the loop in Go, while every linear step
is its own transition.)

Full rationale and examples: [Composition doc](https://docs.kuetix.com/docs/wsl/composition).

## Guidelines
- **Decompose into single-purpose transitions and compose them in WSL — never one fat method (see rule above)**
